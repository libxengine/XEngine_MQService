...
XHTHREAD CALLBACK MessageQueue_DDSMessage_ThreadDomain()
{
	...
	while (bIsRun)
	{
		...
		if (size_t(nMsgLen) >= sizeof(XENGINE_PROTOCOLHDR))
		{
			...
			if (XENGINE_COMMUNICATION_PROTOCOL_OPERATOR_CODE_DDS_REQTOPICCREATE == st_ProtocolHdr.unOperatorCode)
			{
				memcpy(&st_DDSProtocol, tszMsgBuffer + sizeof(XENGINE_PROTOCOLHDR), sizeof(XENGINE_PROTOCOL_XDDS));
				...
				XLOG_PRINT(xhLog, XENGINE_HELPCOMPONENTS_XLOG_IN_LOGLEVEL_INFO, _T("消息分发端:%s,请求创建主题成功,主题名:%s"), tszClientAddr, st_DDSProtocol.tszTopic);
			}
			else if (XENGINE_COMMUNICATION_PROTOCOL_OPERATOR_CODE_DDS_REQTOPICDELETE == st_ProtocolHdr.unOperatorCode)
			{
				memcpy(&st_DDSProtocol, tszMsgBuffer + sizeof(XENGINE_PROTOCOLHDR), sizeof(XENGINE_PROTOCOL_XDDS));
				...
				NetCore_BroadCast_Send(hSDSocket, tszMsgBuffer, nMsgLen);
				XLOG_PRINT(xhLog, XENGINE_HELPCOMPONENTS_XLOG_IN_LOGLEVEL_INFO, _T("消息分发端:%s,请求删除主题成功,主题名:%s"), tszClientAddr, st_DDSProtocol.tszTopic);
			}
			else if (XENGINE_COMMUNICATION_PROTOCOL_OPERATOR_CODE_DDS_REQTOPICQUERY == st_ProtocolHdr.unOperatorCode)
			{
				int nListCount = 0;
				...
				NetCore_BroadCast_Send(hSDSocket, tszMsgBuffer, nMsgLen);
				XLOG_PRINT(xhLog, XENGINE_HELPCOMPONENTS_XLOG_IN_LOGLEVEL_INFO, _T("消息分发端:%s,请求获取主题信息成功,主题名:%s"), tszClientAddr, st_DDSProtocol.tszTopic);
